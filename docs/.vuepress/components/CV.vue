    ...
    <header>
      <div class="line">
        ...
        <div class="line" v-for="item in info.slice(1)" :key="item.icon">
          ...
          <a target="_blank" v-if="item.href" :href="item.href">{{
            item.content
          }}</a>
        </div>
      </div>
      ...
      <div v-for="item in cv" :key="item.title">
        ...
        <ul>
          <li v-for="li in item.list" :key="li[0]">
            <div class="flexBox">
              <div v-for="msg in li" :key="msg">{{ msg }}</div>
            </div>
          </li>
        </ul>
      </div>
...
export default {
      ...
      cv: [
        {
          ...
          list: [["武汉理工大学", "计算机科学与技术", "2018 - 2022"]],
        },
        {
          title: "Intern & Working experience",
          icon: "iconshixi",
          list: [["微派"], ["Shopee"], ["阿里巴巴"]],
        },
        {
          ...
          icon: "iconbiancheng",
          list: [
            ["HTML/CSS/JavsScript/TS"],
            ["Vue/Vuex/VueRouter"],
            ["React/Redux/react-router-dom/RN"],
            ["Node/Koa"],
          ],
        },
        {
          ...
          icon: "iconkeyan",
          list: [
            ["全国大学生交通运输科技大赛", "国家一等奖", "2020"],
            ["中国大学生计算机设计大赛", "中南赛区一等奖", "2020"],
            ["国家级大学生创新训练计划", "负责人", "2020 - 2021"],
            ["软件著作权", "两项", "2020 - 2021"],
          ],
        },
        {
  ...
  computed: {
    ...
    workingTime() {
      ...
      return Number((((100 * (now - start)) / (end - start)) % 100).toFixed(2));
    },
  },
  methods: {
    goMain() {
    ...
    workingYear(percentage) {
      const now = new Date();
      const start = new Date("2022-7-26");
      console.log("now - start", now - start);
      return `第 ${Math.ceil((now - start) / 31536000000)} 年`;
    },
  },
};
</script>
